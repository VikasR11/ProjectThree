...
public class CS400Graph<T> implements GraphADT<T> {
    ...
    protected class Path implements Comparable<Path> {
        ...
        public Path(Path copyPath, Edge extendBy) {
        	...
        	this.distance = copyPath.distance + extendBy.weight; // adding on to distance 
        	this.dataSequence = new LinkedList<>(); // creating an independent LinkedList 
        	...
        	this.dataSequence.add(extendBy.target.data); // adding the last data value 
    ...
    protected Path dijkstrasShortestPath(T start, T end) throws NoSuchElementException {    
    	...
    	if (!this.containsVertex(start) || !this.containsVertex(end))
    		throw new NoSuchElementException("No verticies containing start and/or end values exist");
    	PriorityQueue<Path> queue = new PriorityQueue<Path>(); // frontier 
    	Path first = new Path(new Vertex(start)); // first possible path (start --> start)
    	Vertex current = this.vertices.get(start); // starting vertex; will be overridden 
    	Vertex last = this.vertices.get(end); // ending vertex
    	Hashtable<Vertex, Integer> distances = new Hashtable<Vertex, Integer>(); // maps visited vertices to the shortest distance to get there from starting vertex 
    	for (Edge e: current.edgesLeaving) // populate frontier
    		queue.add(new Path(first, e));
    	Path shortestPath = first; // stores shortest path from start to end
    	distances.put(current, first.distance); // marking starting vertex as visited
    	while (!queue.isEmpty() && current != last) { // We keep iterating till the queue is empty or if we reach the final vertex 
    		Path currentPath = queue.peek(); 
    		current = queue.poll().end; // popping the head of the queue 
    		if (distances.containsKey(current)) // We don't need to operate on already visited vertices 
    			continue; // skips an iteration
    		if (current != last) { // update queue only if current isn't the ending vertex 
    			for (Edge e: current.edgesLeaving) // update frontier
    				queue.add(new Path(currentPath, e));				
    		}
    		shortestPath = currentPath; // update shortest path
    		distances.put(current, shortestPath.distance); // add current to hash table with its distance
    	}		
    	if (!distances.containsKey(last)) // throw NSEE if ending vertex was never visited 
    		throw new NoSuchElementException("Cannot find path between given values"); 		
        return shortestPath; 
    }
