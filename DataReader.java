  ...
  public ArrayList<AirportInterface> readDataSet(Reader inputFileReader)
    ...
    if(temp != null) {
      ...
      while(scan.hasNextLine()) {
        ...
        double cost = Double.parseDouble(header[0].substring(1)); // cost of the flight
        int distance = (int)Double.parseDouble(header[8]); // distance of the flight
        // if the origin airport is not in the hashtable
        ...
        } else { // if the origin airport is in the hashtable already
    ...
    // converting the hashtable into an arraylist
    ArrayList<AirportInterface> listAirports = new ArrayList<AirportInterface>(hash.values());
    // returning the arraylist of AirportInterface objects 
    return listAirports;
  }
}
